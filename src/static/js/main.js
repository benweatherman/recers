...
function setupPlayers() {
  ...
  const addRow = () => {
    ...
    newRow.querySelectorAll("[data-storage-key]").forEach(el => {
      el.dataset.storageKey = el.dataset.storageKey.replace(
        /players\.\d+/,
        `players.${currentRowCount}`
      );
...
function setPageFromID(id) {
  ...
  state.currentPage = state.pages.findIndex(
    pageInfo => pageID === pageInfo.contentID
  );
